...
Floor statistics use all plans in the file rather than dataset sampling. Layout views select a plan. Room numbers are local display indices; apartment colors derive from `unit_id`. The five layout plots have an Apartment selector (CSV unit IDs), defaulting to All apartments. Selecting a unit redraws the plot with other rooms in gray, without changing cached calculations. Statistics remain floor-wide. Plan or visualization changes reset the selector. Room filtering is not implemented.

Window height editing is available on attribute maps, window orientation, window head heights, and both WWR charts. The form shows current CSV height values and pre-fills the new height with the selected value. For mixed heights, choose a group first. **Apply window height change** updates all matching window records across the floor, clears calculated plan caches, and redraws the current visualization. Enter a finite positive height in metres. Missing heights appear as a separate group with an empty input. Changes persist across chart selections in browser memory; reloading the CSV restores its original values. The file on disk is unchanged. Orientation and attribute-map window vectors may change when an edit changes which associated window has the largest area.
